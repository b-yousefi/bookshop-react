...
class BookList extends Component {
  ...
  create_item(key, book) {
    ...
    if (
      this.props.booksInShoppingCart &&
      this.props.booksInShoppingCart.has(book.id)
    ) {
                ...
                >
                  <span>
                    <IconButton
                      ...
                      onClick={this.removeFromCartClicked(
                        book,
                        book_order_count - 1
                      )}
                    >
  ...
  render() {
    const { classes } = this.props;
    if (this.props.books.refreshing) {
      return (
        <Box
          flexGrow={1}
          display="flex"
      ...
      <React.Fragment>
        ...
        >
          {this.props.books.page && this.props.books.page.totalPages > 1 && (
            <Pagination
              color="primary"
              count={this.props.books.page.totalPages}
...
function mapDispatchToProps(dispatch) {
  return bindActionCreators(
    { updateShoppingCart, filterBooksByPage },
    dispatch
  );
}
